import React from "react";
import Line from "../assets/Line 11.png";
import Circle from "../assets/Circle 7.png";
const Why = () => {
  return (
    <div className="w-[100%] h-auto bg-[#F6F9FF] mt-20 ">
      <div className="w-[90%] mx-auto lg:relative lg:z-10 ">
        <h1 className=" pt-20 text-[24px] leading-6  font-black text-[#000735]">
          Why Sanchy Tech
        </h1>
        <p className="mt-8 text-[#242424] text-[18px] md:text-60px md:w-[60%] md:mt-4 ">
          Seamless integration with your existing technology stack Quick and
          easy integration with provider, facility, inventory, scheduling, and
          API databases. We take security seriously and we partner with leading
          API providers.
        </p>

        <div className="mt-[71px] lg:grid lg:grid-cols-2 ">
          <div className=" flex lg:flex-row-reverse lg:max-h-[400px]">
            <div>
              <img
                src={Line}
                alt="/"
                className="h-[180px] ml-4 w-[1px] sm:h-32 lg:ml-8"
              />
              <p className="border-2 w-[32px] h-[32px] rounded-[50%] text-center py-auto mr-[5px] lg:ml-4 pt-1 border-[#001EDC] ">
                <span className="text-[16px] text-[#001EDC] text-center my-auto h-auto ">
                  01
                </span>
              </p>
              <img
                src={Line}
                alt="/"
                className=" ml-4 w-[1px] h-[318px] sm:h-[230px] lg:h-[150px] lg:ml-8 "
              />
            </div>

            <div className="bg-[#FFFFFF] mb-16 lg:mt-[40px] ">
              <div className="w-[90%] mx-auto h-auto pb-16  lg:pb-9 ">
                <h2 className="text-[24px] leading-8 w-[60%] text-[#001EDC] pt-16 pb-4 lg:w-[100%] lg:pt-9">
                  Challenge the ordinary
                </h2>
                <p className="text-[18px] leading-8 text-[#242424]">
                  We are creative people that dare to do things differently. We
                  think critically about tech, the industries we work with, and
                  our society, and look for uncharted paths to have a positive
                  impact.
                </p>
              </div>
            </div>
          </div>

          <div className="flex  lg:max-h-[450px] lg:mt-[300px]">
            <div className="lg:ml-[-37px]">
              <img
                src={Line}
                alt="/"
                className="h-[180px] w-[1px] ml-4 sm:h-32 "
              />
              <p className="border-2 w-[32px] h-[32px] border-[#001EDC] rounded-[50%] text-center py-auto mr-[5px] pt-1">
                <span className="text-[16px] text-[#001EDC] text-center my-auto">
                  02
                </span>
              </p>
              <img
                src={Line}
                alt=""
                className=" ml-4 w-[1px] h-[380px] sm:h-[280px] lg:h-[300px] "
              />
            </div>
            <div className="bg-[#FFFFFF] mb-16 lg:mt-28 lg:ml-4 ">
              <div className="w-[90%] mx-auto pb-16 lg:pb-9">
                <h2 className="text-[24px] leading-8  text-[#001EDC] pt-16 pb-4 lg:pt-9">
                  Grow together
                </h2>
                <p className="text-[18px] leading-8 text-[#242424] ">
                  Opportunities and challenges are both shared here, and when
                  those arise, we tapon each other's strengths to succeed
                  through collaboration. We learn from each other and help each
                  other learn. Lift each other’s voice We work daily on creating
                  a space where we all feel heard.
                </p>
              </div>
            </div>
          </div>

          <div className="flex lg:flex-row-reverse lg:max-h-[400px]">
            <div>
              <img
                src={Line}
                alt="/"
                className="h-[180px] w-[1px] ml-4 sm:h-32 "
              />
              <p className="border-2 w-[32px] h-[32px] rounded-[50%] border-[#001EDC] text-center py-auto mr-[5px] pt-1">
                <span className="text-[16px] text-[#001EDC] text-center my-auto">
                  03
                </span>
              </p>
              <img
                src={Line}
                alt=""
                className=" ml-4 w-[1px] h-[288px] sm:h-[210px]"
              />
            </div>
            <div className="bg-[#FFFFFF] mb-16 pb-16 lg:pb-9 lg:mt-28 lg:mr-4">
              <div className="w-[90%] mx-auto h-auto">
                <h2 className="text-[24px] leading-8  text-[#001EDC] pt-16 pb-4 lg:pt-9">
                  Lift eachother’s voice
                </h2>
                <p className="text-[18px] leading-8 text-[#242424]">
                  We’ve adopted communication routines that encourage the team
                  to share their feedback on projects, internal processes and
                  operations, and anything else.
                </p>
              </div>
            </div>
          </div>

          <div className="flex lg:max-h-[400px] lg:mt-[300px] h-auto">
            <div className="lg:ml-[-37px]">
              <img
                src={Line}
                alt="/"
                className="h-[180px] w-[1px] ml-4 sm:h-32"
              />
              <p className="border-2 w-[32px] h-[32px]  border-[#001EDC] rounded-[50%] text-center py-auto mr-[5px] pt-1">
                <span className="text-[16px] text-[#001EDC] text-center my-auto">
                  04
                </span>
              </p>
              <img
                src={Line}
                alt=""
                className=" ml-4 w-[1px] h-[200px] sm:h-[200px] "
              />
            </div>
            <div className="bg-[#FFFFFF] mb-16 pb-16 lg:pb-9 lg:mt-28 lg:ml-4">
              <div className="w-[90%] mx-auto">
                <h2 className="text-[24px] leading-8 text-[#001EDC] pt-16 pb-4 lg:pt-9">
                  Great collaboration
                </h2>
                <p className="text-[18px] leading-8 text-[#242424]">
                  We do some of our best work through collaboration. We tap on
                  each other’s strengths and help each other grow every day.
                </p>
              </div>
            </div>
          </div>

          <div className="flex lg:flex-row-reverse lg:max-h-[400px] lg:mt-[-40px] h-auto">
            <div>
              <img
                src={Line}
                alt="/"
                className="h-[180px] w-[1px] ml-4 sm:h-32 "
              />
              <p className="border-2 w-[32px] h-[32px] border-[#001EDC] rounded-[50%] text-center py-auto mr-[5px] pt-1">
                <span className="text-[16px] text-[#001EDC] text-center my-auto">
                  05
                </span>
              </p>
              <img
                src={Line}
                alt=""
                className=" ml-4 w-[1px] h-[100px] sm:h-[110px] lg:h-[130px]"
              />
            </div>

            <div className="bg-[#FFFFFF] mb-16 pb-16 lg:pb-9 lg:mt-28 lg:mr-4">
              <div className="w-[90%] mx-auto">
                <h2 className="text-[24px] leading-8 text-[#001EDC] pt-16 pb-4 lg:pt-9">
                  Enjoy the ride
                </h2>
                <p className="text-[18px] leading-8 text-[#242424]">
                  We love to have fun with what we do! The office is where we
                  get to share and follow our passion for tech, design, and for
                  making great new products happen.
                </p>
              </div>
            </div>
          </div>
        </div>
      </div>
      <div className="w-[100%]">
        <img
          src={Circle}
          alt="/"
          className="hidden lg:block w-[35%] absolute ml-[65%] h-auto mt-[-53%]"
        />
      </div>
    </div>
  );
};

export default Why;
